...
export default createStore({
  ...
  getters: {
    allGames : this.state => this.state.games
  },
  actions: {
    async fetchGames({ commit }) {
      const req = await fetch("https://free-to-play-games-database.p.rapidapi.com/api/games", state.options)
        .then(aux => (aux.json()))
  ...
  mutations: {
    ...
    updatePlatform : (state, payload) => {
      ...
      if (platform=="NotSelected") {
        ...
        if (state.selectedGenre) {
          state.games = state.games.filter((game) => (game.platform === platform && game.genre === this.selectedGenre))
        } else {
